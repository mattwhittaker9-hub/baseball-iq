...
function ensurePressureBar(){
  if(document.getElementById('clutch-pw'))return;
  var wrap=document.createElement('div');wrap.className='clutch-pw';wrap.id='clutch-pw';
  wrap.innerHTML='<div class="clutch-pb"><div class="clutch-pf" id="clutch-pf"></div></div><div class="clutch-pl" id="clutch-pl"></div>';
  // Try to insert before daily game card
  var target=document.querySelector('.daily-gc-card')||document.querySelector('#daily-gc-scenario')||document.querySelector('.s-daily-game');
...
function hookAnswers(){
  ...
  wrapFn("recordDailyAnswer",function(ok){onAnswer(ok===true);onDailyAnswer(ok===true);});
  wrapFn("showCardFeedback",function(ok){onAnswer(ok===true);if(_isDailyMode)ensurePressureBar();});
}
// ── DAILY STREAK LOGIC ──────────────────────────────────────────
var _dailyStreak=0;
var _isDailyMode=false;

function onDailyAnswer(ok){
  if(ok){
    _dailyStreak++;
  } else {
    _dailyStreak=0;
  ...
  // Build pressure based on streak: 1=0%, 2=20%, 3=50%, 4=80%, 5=100%
  var pct=[0,0,20,50,80,100][Math.min(_dailyStreak,5)];
  setDailyPressure(pct,_dailyStreak);
  if(_dailyStreak>=5){
    // Perfect session — full walkoff
    haptic([100,50,100,50,200,50,200]);
    setTimeout(function(){showWalkoff();},400);
  } else if(_dailyStreak>=4){
    haptic([30,100,30,100,80]);
  } else if(_dailyStreak>=2){
    haptic([30,80,30]);
  }
}

function setDailyPressure(pct,streak){
  // Ensure vignette and pressure exist
  if(!CM.vignetteEl)return;
  if(pct===0){
    CM.vignetteEl.style.opacity='0';
    var card=document.querySelector('.daily-gc-card')||document.querySelector('.gc-card');
    if(card)card.classList.remove('clutch-cp');
    var sb=document.getElementById('clutch-sb');if(sb)sb.classList.remove('show');
  } else {
    // Scale vignette intensity with streak
    var intensity=pct/100*0.6;
    CM.vignetteEl.style.opacity=String(intensity);
    CM.vignetteEl.style.background='radial-gradient(ellipse at center,transparent 35%,rgba(160,20,20,'+intensity+') 100%)';
    if(pct>=80){
  ...
  // Update pressure bar
  var fill=document.getElementById('clutch-pf');
  if(fill)fill.style.width=pct+'%';
  var label=document.getElementById('clutch-pl');
  if(label){
    if(streak>=4){label.textContent='\u26a1 '+streak+' in a row — one more!';label.classList.add('show');}
    else if(streak>=2){label.textContent=streak+' in a row';label.classList.add('show');}
    else label.classList.remove('show');
  }
}

function resetDailyPressure(){
  _dailyStreak=0;
  if(CM.vignetteEl){CM.vignetteEl.style.opacity='0';CM.vignetteEl.classList.remove('active');}
  var card=document.querySelector('.daily-gc-card')||document.querySelector('.gc-card');
  if(card)card.classList.remove('clutch-cp');
  var sb=document.getElementById('clutch-sb');if(sb)sb.classList.remove('show');
  var fill=document.getElementById('clutch-pf');if(fill)fill.style.width='0%';
  var label=document.getElementById('clutch-pl');if(label)label.classList.remove('show');
}

function buildDailySB(streak){
  var sb=document.getElementById('clutch-sb');
  if(!sb){
    sb=document.createElement('div');sb.className='clutch-sb';sb.id='clutch-sb';
    var card=document.querySelector('.daily-gc-card')||document.querySelector('#daily-gc-scenario');
    if(card&&card.parentNode)card.parentNode.insertBefore(sb,card);
    else document.body.appendChild(sb);
  }
  var left=5-streak;
  sb.innerHTML='<div class="csb-inn">\u26a1 '+(streak)+' IN A ROW</div>'
    +'<div class="csb-score" id="csb-score">'+(left>0?left+' more for a perfect session':'PERFECT SESSION!')+'</div>'
    +'<div class="csb-dots" id="csb-dots"></div>';
  // Draw dots
  var dots=sb.querySelector('#csb-dots');
  if(dots){dots.innerHTML='';for(var i=0;i<5;i++){var d=document.createElement('div');d.className='csb-dot'+(i<streak?' ok':'');dots.appendChild(d);}}
  sb.classList.add('show');
}

function showWalkoff(){
  var o=document.getElementById('lu-overlay');if(!o)return;
  document.getElementById('lu-trophy').textContent='\uD83C\uDFC6';
  document.getElementById('lu-name').textContent='PERFECT SESSION';
  document.getElementById('lu-sub').textContent='5 for 5. That\'s a walk-off.';
...
window._onDailyAnswerDirect=function(ok){if(_isDailyMode)onDailyAnswer(ok);};window._setDailyMode=function(on){
  _isDailyMode=!!on;
